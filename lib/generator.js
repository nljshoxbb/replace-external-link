...
class Generator {
    ...
    getConfigFile = async () => {
        if (fs.existsSync(path.join(process.cwd(), constant_1.CONFIG_FILE_NAME))) {
            const data = require(path.join(process.cwd(), constant_1.CONFIG_FILE_NAME));
    ...
    processFiles = async (files) => {
        for (const filePath of [...files]) {
            ...
            if (!extname) {
                ...
                await (0, utils_1.writeFile)(replacedPath, replacedContent);
            }
        }
    ...
    scanDir = () => {
        /** 扫描当前文件 */
        const results = (0, glob_1.globSync)(`${this.sourceDir}/**`, {
            stat: true,
            withFileTypes: true,
            ignore: ["node_modules/**", ".lock"],
        });
        const periodResults = (0, glob_1.globSync)("**/.**/**", {
            stat: true,
            withFileTypes: true,
            ignore: ["node_modules/**", ".lock"],
        });
    ...
    fetchStaticResources = async (urls) => {
        ...
        for (const url of urlSet) {
            ...
            input.push(limit(() => (0, utils_1.downloadFile)(url, destPath)));
        }
        try {
